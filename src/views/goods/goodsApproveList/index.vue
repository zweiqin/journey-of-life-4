	...
	<div class="app-container">
		...
		>
						...
						<el-form-item label="宣传画廊">
							<div v-if="scope.row.gallery && scope.row.gallery.length">
								<el-image
									...
									:preview-src-list="JSON.parse(scope.row.gallery).map(i => common.seamingImgUrl(i))"
								/>
							</div>
							<span v-else>--</span>
						</el-form-item>
			...
			<el-table-column align="center" property="iconUrl" label="图片">
				<template slot-scope="scope">
					<el-image
						v-if="scope.row.picUrl" :src="common.seamingImgUrl(scope.row.picUrl)" style="width:40px; height:40px"
						fit="cover" :preview-src-list="[ common.seamingImgUrl(scope.row.picUrl) ]"
					/>
					<span v-else>--</span>
				</template>
			</el-table-column>
			...
			<el-table-column align="center" label="操作" fixed="right" width="80" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<el-button
						:disabled="scope.row.approveStatus !== 0" type="warning" size="mini"
		...
		<el-dialog title="商品审核" :visible.sync="dialogFormVisibleExamine" width="800px">
			<el-form
				...
				label-position="left"
				label-width="80px" style="width: 100%; padding:0 35px;"
			>
				<el-form-item label="审核状态" prop="status">
					<el-select v-model="dataFormExamine.status" class="filter-item">
						<el-option label="审批通过" :value="1" />
						<el-option label="审批拒绝" :value="2" />
					</el-select>
				</el-form-item>
...
<script>
import { approveGoodsList, goodsApprove } from '@/api/business/goods'
...
export default {
	...
	methods: {
		getList() {
			...
			approveGoodsList(this.listQuery).then((response) => {
