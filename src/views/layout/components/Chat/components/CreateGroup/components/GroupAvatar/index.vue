  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf('image/') == -1) {
        this.$message({ showClose: true, message: '文件格式错误，请上传图片类型,如：JPG，PNG后缀的文件。', type: 'error' })
      } else {
    ...
    // 上传图片
    uploadImg() {
      if (this.previews.url == '' || this.previews.url == undefined) { this.$parent.$parent.uploadAvatarSuccess() }
