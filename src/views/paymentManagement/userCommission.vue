<template>
	<div class="app-container">
		...
		<div class="filter-container">
			...
			/>
			<el-select
				v-model="listQuery.statusArray" multiple size="small" style="width: 200px"
				class="filter-item"
				placeholder="请选择提现状态"
			>
			...
			<el-button
				class="filter-item" type="primary" size="small" icon="el-icon-search"
				@click="handleFilter"
			>
				查找
			</el-button>
		...
		<div v-tableHeight>
			...
			>
				...
				<el-table-column align="center" label="操作" width="120" class-name="small-padding fixed-width">
					<template slot-scope="scope">
						<el-button
							v-if="scope.row.status == 0 || scope.row.status == 2" v-permission="[ 'GET /admin/brokerage/approve' ]"
							type="primary" size="mini" @click="handleApprove(scope.row)"
						>
							审批
						</el-button>
						<el-button v-else v-permission="[ 'GET /admin/brokerage/approve' ]" type="info" size="mini">已审批</el-button>
					</template>
				</el-table-column>
		...
		<el-dialog :visible.sync="approveDialogVisible" title="提现审批">
			<el-form
				ref="approveForm" :rules="rules" :model="approveForm" status-icon
...
<script>
import { fetchList, approveTrace } from '@/api/paymentManagement/brokerage'
