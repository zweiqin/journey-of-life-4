<template>
	<div class="base">
		<el-card :style="{ height: '100%', width: '100%' }">
			<div v-if="UserInfo.admin" :style="{ fontSize: 20 * $root.dw + 'px' }" class="admindiv">
				<p>账号信息（PC端）</p>
				<div>
					<img
						:style="{ borderRadius: '50%' }" :src="common.seamingImgUrl(UserInfo.admin.avatar)" width="80px"
						alt=""
					>
				</div>
				<ul>
			...
			<div v-else>暂无信息</div>
			<el-divider />
			<div v-if="UserInfo.user" :style="{ fontSize: 20 * $root.dw + 'px' }" class="admindiv">
				<p>用户信息（移动端）</p>
				<div>
					<img :style="{ borderRadius: '50%' }" :src="common.seamingImgUrl(UserInfo.user.avatar)" width="80px" alt="">
				</div>
				<ul>
			...
			<div v-else>暂无信息</div>
			<el-divider />
			<div v-if="UserInfo.salesman" :style="{ fontSize: 20 * $root.dw + 'px' }" class="admindiv">
				<p>身份信息</p>
				<ul>
					...
					<li>身份证正面：</li>
					<img
						v-if="UserInfo.salesman.idcardProsUrl" :style="{ borderRadius: '50%' }"
						:src="common.seamingImgUrl(UserInfo.salesman.idcardProsUrl)" width="80px" alt=""
					>
					<li>身份证反面：</li>
					<img
						v-if="UserInfo.salesman.idcardConsUrl" :style="{ borderRadius: '50%' }"
						:src="common.seamingImgUrl(UserInfo.salesman.idcardConsUrl)" width="80px" alt=""
					>
			...
			<div v-else>暂无信息</div>
			<el-divider />
			<div v-if="UserInfo.voucherHold" :style="{ fontSize: 20 * $root.dw + 'px' }" class="admindiv">
				<p>代金券信息</p>
				<ul>
